...
function authenticate_userpass(creds::UserPasswordCredentials, cred::Ptr{Ptr{Void}}, schema, host, urlusername)
    isusedcreds = checkused!(creds)
    ...
    try
        ...
        if isset(allowed_types, Cuint(Consts.CREDTYPE_SSH_KEY))
            ...
            if isa(sshcreds, SSHCredentials)
                ...
                err = authenticate_ssh(creds, cred, username_ptr, schema, host)
                err == 0 && return err
            end
        end
        ...
        if isset(allowed_types, Cuint(Consts.CREDTYPE_USERPASS_PLAINTEXT))
            ...
            return authenticate_userpass(creds, cred, schema, host, urlusername)
        end
